...
- Pala installed and running ([Installation guide](/getting-started/installation))
...
<Step title="Initialize the site">
  When you first access Pala, create a new site. Give it a descriptive name like "My First Site".
...
<Step title="Write the component">
  ...
  <script>
    ...
    export let subheadline = "Your compelling subheadline here";
    export let cta_text = "Get Started";
    export let cta_link = "/";
    export let background_image = "";
  </script>
  
  <section class="hero" style={background_image ? `background-image: url(${background_image})` : ''}>
...
<Step title="Add fields">
  ...
  - `image_url` (Image) - The image to display
  - `alt_text` (Text) - Alt text for accessibility
  - `caption` (Text, optional) - Image caption
  - `full_width` (Boolean) - Whether to use full width
</Step>
</Steps>
...
<Step title="Write the component">
  ```svelte
  <script>
    export let copyright_text = "© 2024 My Site";
    export let links = [];
  </script>
...
<Step title="Configure available blocks">
  ...
  <Info>
  The Footer will be added site-wide, so we don't need it in individual page types.
  </Info>
</Step>
...
<Step title="Add menu items">
  ...
  - Blog (links to blog listing, if you create one)
  
  <Tip>
  Navigation is managed at the site level, so it appears consistently across all pages.
  </Tip>
</Step>
</Steps>

## Step 6: Set Up Site-Wide Footer

<Steps>
<Step title="Configure footer">
  Go to Site Settings → Footer.
</Step>

<Step title="Add footer content">
  Use the Footer block settings to add:
  - Copyright text
...
<Card title="Using the Editor" icon="pen-to-square" href="/content-management/using-the-editor">
  Learn how content editors use Pala to manage pages.
</Card>
</CardGroup>
